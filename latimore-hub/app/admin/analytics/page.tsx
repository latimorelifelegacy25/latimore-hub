...
export default async function AnalyticsPage() {
  const [sourceCounts, countyCounts, recentEvents, productCounts] = await Promise.all([
    prisma.inquiry.groupBy({ by: ['source'], _count: { _all: true }, where: { source: { not: null } }, take: 10 }),
    prisma.contact.groupBy({ by: ['county'], _count: { _all: true }, where: { county: { not: null } }, take: 10 }),
    prisma.systemEvent.findMany({ take: 25, orderBy: { occurredAt: 'desc' } }),
    prisma.inquiry.groupBy({ by: ['productInterest'], _count: { _all: true }, take: 10 }),
  ])
