...
export function InquiryNotification(p: {
  ...
  const name = `${p.firstName ?? ''} ${p.lastName ?? ''}`.trim() || 'New Lead'
  return `
  <div style="font-family:Arial,sans-serif;font-size:14px;line-height:1.6;color:#0B0F17">
    ...
    <div style="background:#ffffff;padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px">
      ...
      <table style="width:100%;border-collapse:collapse">
        ...
        <tr><td style="padding:8px 0;color:#6b7280">Interest</td><td style="padding:8px 0"><span style="background:#C9A25F;color:#0B0F17;padding:2px 10px;border-radius:999px;font-size:12px;font-weight:700">${p.interestType ?? 'Unknown'}</span></td></tr>
