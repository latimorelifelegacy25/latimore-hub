...
async function copyFileWithDedup(src, destDir, baseName){ await fsp.mkdir(destDir,{recursive:true}); let out=path.join(destDir,baseName); const {name,ext}=path.parse(baseName); let i=1; while(fs.existsSync(out)){ out=path.join(destDir,`${name}-${i}${ext}`); i++; } await fsp.copyFile(src,out); return out; }
